...
interface PugIconProps
  extends React.PropsWithoutRef<Omit<React.ComponentProps<"svg">, "viewBox">> {
  gradient?: { stopColor: string; offset: string }[];
}

const PugIcon: React.FC<PugIconProps> = ({ gradient, ...props }) => {
  let grad;
  if (gradient != undefined) {
    grad = (
      <linearGradient id="PugIconGradient">
        {gradient.map((gr, key) => (
          <stop stopColor={gr.stopColor} offset={gr.offset} key={key} />
        ))}
      </linearGradient>
    );
    props.fill = "url(#PugIconGradient)";
  } else {
    grad = <></>;
  }

  return (
    <svg viewBox="0 0 38.95 31.66" {...props}>
      {grad}
