...
interface AussieIconProps
  extends React.PropsWithoutRef<Omit<React.ComponentProps<"svg">, "viewBox">> {
  gradient?: { stopColor: string; offset: string }[];
}

const AussieIcon: React.FC<AussieIconProps> = ({ gradient, ...props }) => {
  let grad;
  if (gradient != undefined) {
    grad = (
      <linearGradient id="AussieIconGradient">
        {gradient.map((gr, key) => (
          <stop stopColor={gr.stopColor} offset={gr.offset} key={key} />
        ))}
      </linearGradient>
    );
    props.fill = "url(#AussieIconGradient)";
  } else {
    grad = <></>;
  }

  return (
    <svg viewBox="0 0 36.38 32.19" {...props}>
      {grad}
